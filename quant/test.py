...
def test_bmm_speed():
    ...
    query_state = torch.randn(
        (BS, nh, query_len, D), device="cuda", dtype=torch.float16
    )
    ...
    attn_weight = torch.randn(
        (BS, nh, query_len, T), device="cuda", dtype=torch.float16
    )
    code, scale, mn = triton_quantize_and_pack_along_last_dim(
...
def test_streaming_kvcache():
    ...
    for i in range(16):
        ...
        key_states_new = torch.randn(
            (BS, nh, query_len, D), device="cuda", dtype=torch.float16
        )
        value_states_new = torch.randn(
            (BS, nh, query_len, D), device="cuda", dtype=torch.float16
        )
        ...
        print(
            f"i {i} bit {bits}, mean att weight rel arr: {torch.mean(torch.abs(att_weight_gap))}"
        )
        att_out_gap = (ref_att_out - our_attn_output) / ref_att_out
        print(
            f"i {i} bit {bits}, mean att out rel arr: {torch.mean(torch.abs(att_out_gap))}"
        )
...
def test_4d_qmatmul():
    ...
    query_state = torch.randint(5, (BS, nh, query_len, D), device="cuda").to(
        torch.float16
    )
    for bits in [8, 4, 2]:
