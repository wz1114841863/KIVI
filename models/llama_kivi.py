    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = self.q_proj(hidden_states)  # 计算Q, [1, 405, 768]
            key_states = self.k_proj(hidden_states)
            value_states = self.v_proj(hidden_states)
        ...
        if past_key_value is not None:
            ...
            if key_states_quant_trans is not None:
                att_qkquant = cuda_bmm_fA_qB_outer(
                    self.group_size,
                    query_states,  # [1, 12, 1, 64]
                    key_states_quant_trans,  # [1, 12, 64, 24]
                    key_scale_trans,  # [1, 12, 64, 12]
                    key_mn_trans,  # [1, 12, 64, 12]
                    self.k_bits,  # 2
                )  # [1, 12, 1, 384]
                ...
                att_qkquant = None
            if key_states_full is not None:  # 拼接高精度部分
                key_states_full = torch.cat([key_states_full, key_states], dim=2)
            else:
                key_states_full = key_states
            ...
            if key_states.shape[-2] % self.residual_length != 0:
                if key_states.shape[-2] < self.residual_length:
                    ...
                    key_states_quant = key_states[
                        :, :, : -(key_states.shape[-2] % self.residual_length), :
                    ].contiguous()  # 前面的量化部分, [1, 12, 384, 64]
                    key_states_full = key_states[
                        :, :, -(key_states.shape[-2] % self.residual_length) :, :
                    ].contiguous()  # 后面的残差部分, 高精度保存, [1, 12, 21, 64]
            else:
                key_states_quant = key_states
                key_states_full = None
            if key_states_quant is not None:
                ...
                )  # key_states_quant_trans: [1, 12, 64, 24], int8; key_scale_trans: [1, 12, 64, 12], float16; key_mn_trans: [1, 12, 64, 12], float16
            else:
                key_states_quant_trans = None
                key_scale_trans = None
                key_mn_trans = None
            ...
            if value_states.shape[-2] <= self.residual_length:
                ...
                value_states_quant = value_states[
                    :, :, : -self.residual_length, :
                ].contiguous()  # [1, 12, 373, 4]
                value_states_full = value_states[
                    :, :, -self.residual_length :, :
                ].contiguous()  # [1, 12, 32, 64]
                value_states_quant, value_scale, value_mn = (
                    triton_quantize_and_pack_along_last_dim(
                        value_states_quant, self.group_size, self.v_bits
                    )
                )  # value_states_quant: [1, 12, 373, 4], int8; value_scale: [1, 12, 373, 2], float16; value_mn: [1, 12, 373, 2], float16
